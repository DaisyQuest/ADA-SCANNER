...
/// <summary>
/// Checks for tables that appear to be used for layout.
/// </summary>
public sealed class LayoutTableCheck : ICheck
{
    ...
    private static readonly Regex CaptionRegex = new("<caption\\b", RegexOptions.IgnoreCase | RegexOptions.Compiled);

    /// <inheritdoc />
    public string Id => "layout-table";
    ...
    public IEnumerable<Issue> Run(CheckContext context, RuleDefinition rule)
    {
        ...
        foreach (Match match in TableRegex.Matches(context.Content))
        {
            ...
            if (HeaderRegex.IsMatch(content) || CaptionRegex.IsMatch(content))
            {
                continue;
            }

            var line = TextUtilities.GetLineNumber(context.Content, match.Index);
            issues.Add(new Issue(rule.Id, Id, context.FilePath, line, "Table appears to be used for layout.", match.Value));
        }
